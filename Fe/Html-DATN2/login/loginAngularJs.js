...
app3.controller("resetPassCtrcl", function ($scope, $http, $location) {
  $scope.resetPass = function () {
    ...
    $http
      .post("http://localhost:8080/auth/reset-password", data)
...
app2.controller("otpCtrcl", function ($scope, $http, $location) {
  $scope.forgotPassword = function () {
    $http
      .post("http://localhost:8080/auth/forgot-password/" + $scope.email)
      .then(function (response) {
  ...
  $scope.resendOTP = function () {
    $http
      .post("http://localhost:8080/auth/reSendOTP/" + $scope.email)
      .then(function (response) {
  ...
  $scope.submitOTP = function () {
    ...
    $http
      .post("http://localhost:8080/auth/confirm-otp", data)
      .then(function (response) {
...
app.controller("LoginCtrl2", function ($scope, $http, $location) {
  ...
  $scope.registerUser = function () {
    ...
    $http
      .post("http://localhost:8080/account/check-email", {
      ...
      .then(function (emailResponse) {
        if (emailResponse.data.exists) {
          ...
          return alert("Email đã được sử dụng. Vui lòng chọn email khác.");
        } else {
          // Nếu email chưa được sử dụng, tiến hành kiểm tra account
          $http
            .post("http://localhost:8080/account/check-account", {
            ...
            .then(function (accountResponse) {
              if (accountResponse.data.exists) {
                ...
                $http
                  .post("http://localhost:8080/auth/register", $scope.user)
